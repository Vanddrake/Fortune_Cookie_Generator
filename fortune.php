...
<!-- 
Author: Robert Zaranek - Sept 30, 2020 
On this webpage, fortune cookies are displayed according to the user's previous input.
-->

<?php
/* Holds and sterilizes the user's input name */
$name = filter_input(INPUT_GET, "name", FILTER_SANITIZE_SPECIAL_CHARS);
/* Holds and validates the user's input fortune cookie quantity */
$quantity = filter_input(INPUT_GET, "quantity", FILTER_VALIDATE_INT);
/* Holds and sterilizes the user's input colour choice */
$color = filter_input(INPUT_GET, "color", FILTER_SANITIZE_STRING);
/* Holds and validates the user's input minimum fortune cookie lucky number */
$min = filter_input(INPUT_GET, "min", FILTER_VALIDATE_INT);
/* Holds and validates the user's input maximum fortune cookie lucky number */
$max = filter_input(INPUT_GET, "max", FILTER_VALIDATE_INT);
/* Holds a boolean indicating if an invalid data error has occured */
$error = false;

// Checks if input values match the required perameters
if ($min < 1 || $min > 100 || $max < 1 || $max > 100 || $quantity < 1 || $quantity > 50 || strlen($name) > 50 || strlen($color) > 7) {
    $error = true;
}
...
class Cookie
{
    ...
    /**
     * Returns the color of this fortune cookie
     * 
     * @returns The color of this fortune cookie
     */
    ...
    /**
     * Returns a string including a random fortune and 7 random numbers in the user's
     * specified range.
     * 
     * @returns A string including a random fortune and 7 random numbers in the user's specified range
     */
    public function get_fortune()
    {
        $lucky_numbers = rand($this->min, $this->max); // Holds a string of 7 random numbers, initialized with 1 random number

        // Loops 6 times, adding a new random number each time 
        for ($n = 0; $n < 6; $n++) {
            $lucky_numbers = $lucky_numbers . ", " . rand($this->min, $this->max);
        }

        return $this->fortune[rand(0, 352)] . "<br>Lucky #s: " . ($lucky_numbers);
    }
}
?>
...
<head>
    <title>Your Fortune Cookies!</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="fortunestyle.css" rel="stylesheet" type="text/css">
</head>
...
<body>
    <main id="cookie_view">
        ...
        if ($error) {
            echo "<h1 style='color:crimson;'>An invalid value has been entered!</h1>";
        } else {
            echo "<h1>Here are your fortune cookies, $name!</h1>
            <div id='cookie_flexbox'>";

            $fortune_cookies = []; // Holds an array of Cookie objects

            // Populates array with a number Cookie objects equal to the user input amount
            for ($c = 0; $c < $quantity; $c++) {
                array_push($fortune_cookies, new Cookie($color, $min, $max));
            }

            // Displays a fortune cookie picture and fortune for each Cookie object in the array
            foreach ($fortune_cookies as $cookie) {
                echo "<div class='pic_container'>
                <img src='fortune_cookie.png' alt='a picture of a fortune cookie' style='filter: opacity(0.5) drop-shadow(0 0 0 " . $cookie->get_color() . ");'>
                <div class='fortune_message'>
                    " . $cookie->get_fortune() . "
                </div>
            </div>";
            }
